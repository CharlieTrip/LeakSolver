...
const PERMUTATION: [usize; 16] = [0, 5, 10, 15, 13, 2, 3, 4, 9, 14, 12, 7, 1, 6, 8, 11];
const INV_PERMUTATION: [usize; 16] = [0, 12, 5, 6, 7, 1, 13, 11, 14, 8, 2, 15, 10, 4, 9, 3];

impl ParallelHelper<L, AESHelper> for AESInputHelper {
  fn parallel(&self) -> (Vec<Option<AESHelper>>, Vec<Constrain<u8, AESHelper>>) {
    let checksperm = vec![
      vec![0, 1, 2, 3, 4, 0],
      vec![2, 3, 0, 1, 3, 5],
      vec![0, 1, 2, 3, 4, 0, 5],
      ...
      vec![8, 9, 6, 7, 9, 12, 1],
      vec![9, 6, 7, 8, 3, 5, 13],
      vec![14, 4, 5, 11, 4, 0, 7, 14],
      vec![4, 5, 11, 14, 9, 12, 1, 8],
      vec![5, 11, 14, 4, 3, 5, 13, 2],
      vec![11, 14, 4, 5, 10, 6, 11, 15],
      vec![10, 12, 13, 15, 4, 0, 7, 14, 10],
      vec![12, 13, 15, 10, 9, 12, 1, 8, 4],
      vec![13, 15, 10, 12, 3, 5, 13, 2, 9],
      vec![15, 10, 12, 13, 10, 6, 11, 15, 3],
    ];

    let checkindex = [0, 2, 7, 3, 4, 1, 5, 6, 8, 9, 10, 11, 12, 13, 14, 15];

    let skipindex: Vec<usize> = vec![4, 5, 5, 1, 1, 2, 2, 3, 4, 4, 4, 5, 5, 5, 5, 5];

    let fixer: Vec<Option<Vec<usize>>> = vec![
      Some(vec![0, 1, 2, 3, 4, 0]),
      Some(vec![2, 3, 0, 1, 3, 5]),
      Some(vec![6, 7, 8, 9, 10, 6, 11]),
      Some(vec![3, 0, 1, 2, 10, 6]),
      Some(vec![7, 8, 9, 6, 4, 0, 7]),
      Some(vec![1, 2, 3, 0, 9, 12]),
      Some(vec![8, 9, 6, 7, 9, 12, 1]),
      Some(vec![9, 6, 7, 8, 3, 5, 13]),
      Some(vec![14, 4, 5, 11, 4, 0, 7, 14]),
      Some(vec![4, 5, 11, 14, 9, 12, 1, 8]),
      Some(vec![5, 11, 14, 4, 3, 5, 13, 2]),
      Some(vec![11, 14, 4, 5, 10, 6, 11, 15]),
      Some(vec![10, 12, 13, 15, 4, 0, 7, 14, 10]),
      Some(vec![12, 13, 15, 10, 9, 12, 1, 8, 4]),
      Some(vec![13, 15, 10, 12, 3, 5, 13, 2, 9]),
      Some(vec![15, 10, 12, 13, 10, 6, 11, 15, 3]),
    ];
  ...
  fn par_dimensions(&self) -> (Vec<Vec<usize>>, Vec<usize>) {
    (
      vec![vec![0, 5, 10, 15, 13, 2], vec![3, 4, 9, 14, 12, 7]],
      vec![1, 6, 8, 11],
    )
  }
  ...
  fn par_skips(&self) -> (Vec<Vec<usize>>, Vec<usize>) {
    (
      vec![vec![0, 2], vec![7]],
      vec![3, 4, 1, 5, 6, 8, 9, 10, 11, 12, 13, 14, 15],
    )
  }
}
